...
def _validate_responses_input(input_value: Any, source: Model) -> None:
    ...
    for item_index, item in enumerate(input_value):
        ...
        if item.get("type") not in allowed_items:
            _source_unsupported(
                "provider-native Responses input items are outside the supported text/tool profile",
                source,
                path,
            )
        ...
        for part_index, part in enumerate(content):
            ...
            if set(part) - {"type", "text"}:
                _source_unsupported(
                    "Responses text annotations and metadata are outside the supported text/tool profile",
                    source,
                    part_path,
                )
...
def _validate_gemini_generation_config(value: Any, source: Model) -> None:
    ...
    if unknown:
        key = min(unknown)
        _source_unsupported(
            f"Gemini generation option {key!r} is outside the supported text/tool profile",
            source,
    ...
    if candidate_count not in {None, 1}:
        _source_unsupported(
            "multiple Gemini candidates are outside the supported text/tool profile",
            source,
...
def _reject_unknown_source_fields(
    ...
    if unknown:
        _source_unsupported(
            f"{source.api_type.value} field {unknown[0]!r} is outside the supported text/tool profile",
            source,
            f"$.{unknown[0]}",
        )
