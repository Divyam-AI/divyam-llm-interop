...
def _content_block_start(
    ...
    if block_type == "text":
        ...
        if block.get("citations"):
            _unsupported(
                "citations are outside TEXT_TOOL_ROUTING_V1",
...
def _message_delta(event: dict[str, Any], state: SourceStreamState) -> dict[str, Any]:
    ...
    if stop_reason == "pause_turn":
        _unsupported(
            "pause_turn is outside TEXT_TOOL_ROUTING_V1", "$.delta.stop_reason"
        )
...
def _internal_error_chunk(
    ...
    return {
        "id": state.message_id or "msg_divyam_stream_error",
        "object": "chat.completion.chunk",
        "created": state.created,
...
def _internal_anthropic_event_chunk(
    ...
    return {
        "id": state.message_id or "msg_divyam_stream_event",
        "object": "chat.completion.chunk",
        "created": state.created,
        "model": state.model or state.source_model,
        "choices": [],
