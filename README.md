...
APIs are outside the current routing profile and fail closed rather than being
silently flattened.
...
heuristics cannot distinguish them safely. Router integrations must supply the
ingress API type explicitly:
...
Responses conversion behavior is intentionally unchanged by this addition; the
full compatibility boundary is recorded in the implementation contract.
...
The approved scope, translation rules, failure policy, implementation phases,
and test catalogue for native Anthropic Messages support are recorded in
the [Anthropic Messages interop implementation contract](./docs/ANTHROPIC_MESSAGES_INTEROP_IMPLEMENTATION_PLAN.md).
The current implementation follows its `TEXT_TOOL_ROUTING_V1` boundary.
